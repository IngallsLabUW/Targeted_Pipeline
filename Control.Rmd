...
The matching.pattern and software.pattern variables should not be changed by the user.
...
print(paste("Your run type is:", file.pattern))
```
...
skyline.RP.Cyano <- `181112_CyanoAq_MESO-SCOPE_HRM`
...
# Clear environment
rm(list = setdiff(ls()[!ls() %in% c("file.pattern", "currentDate", "software.pattern")], lsf.str()))
```
...
# ID run types
run.types <- IdentifyRunTypes(skyline.output)
--------------------------------------------------------------
